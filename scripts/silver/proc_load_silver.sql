...
Script Purpose:
Stored Procedure ini melakukan ETL (Extract, Transform, Load) dari Bronze Table ke Silver Table
dengan cara : 
  - truncate 
  - insert setelah cleaning data bronze
  - membuat load duration time pertable & batch
...
BEGIN
	...
	BEGIN TRY
		...
		FROM
			(SELECT
				*,
				ROW_NUMBER() OVER(PARTITION BY cst_id ORDER BY cst_create_date DESC) flag -- memakai descending agar nilai 1 (data terupdate) ada di bagian atas
		...
		SELECT 
			CASE 
				WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid,4, LEN(cid)) -- Removing NAS
				ELSE cid
			END cid,
			CASE 
